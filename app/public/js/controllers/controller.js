...
myApp.controller('AppCtrl', ['$scope', '$http','appService','$filter', function($scope, $http,appService,$filter) {
	var vm = this;
    vm.taskList = {};
    var getData = function(){
        appService.getUserData().then(function(response){
            ...
            if(response.data.length == 0){
                ...
                vm.taskList = response.data[0].data;
	...
	vm.show = function(type,item){
		var idx = vm.taskList[type].indexOf(item);
		vm.taskList[type][idx].showDetails = true;

	}
   vm.hide = function(type,item) {
   		var idx = vm.taskList[type].indexOf(item);
		vm.taskList[type][idx].showDetails = false;
        var date = vm.taskList[type][idx].date;console.log(vm.taskList[type][idx].date);
        vm.taskList[type][idx].date = $filter('date')(date, "yyyy-MM-dd");
        saveData(); 
   }
   ...
   vm.addNew = function(type){
        ...
        "showDetails":true,
   		...
   		vm.taskList[type].push(newTask);
   }
   vm.del = function(type,item){
        var idx = vm.taskList[type].indexOf(item);
        vm.taskList[type].splice(idx, 1);
        saveData();
   }
   vm.model = [generateList(1), generateList(2)];
    ...
    function saveData(){
        appService.setUserData(vm.taskList).then(function(response){//console.log(response);
        })
    }
    ...
    e.directive("dndDraggable", ["$parse", "$timeout", function(e, i) {
        ...
        return function(i, l, f) {
            ...
            l.on("dragenter", function(e) {
                ...
                if ("none" == T) return p()
                var h = D()
                return f.dndDrop && (d = v(f.dndDrop, e, T, a, h, d), !d) ? p() : (o.dropEffect = T, o.dropEffect = T, y || (e.dataTransfer.dropEffect = T), d !== !0 && i.$apply(function() {
